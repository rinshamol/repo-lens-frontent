...
const getErrorType = (error: string): ErrorType => {
  ...
  if (e.includes("rate") || e.includes("429") || e.includes("too many"))
    return "ratelimit";
  if (e.includes("500") || e.includes("server") || e.includes("parse"))
    return "server";
  return "generic";
};
...
const ErrorMessage: React.FC<Props> = ({ error, onRetry, onLogin, isLoggedIn }) => {
  ...
  return (
    <div
      className={`rounded-xl border p-4 ${config.bg} ${config.border} animate-in slide-in-from-top-2 duration-300`}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">
            {type === "generic" ? error : config.hint}
          </p>
          {retryCount > 0 && (
        ...
        <div className="flex items-center gap-2 shrink-0">
          {config.showLogin && onLogin && !isLoggedIn &&(
            <button
              onClick={onLogin}
